...
<div id="headWrapper">
  ...
  <div id="message">
   <p><?php echo str_replace('{count}',mswRowCount('tickets WHERE `ticketStatus` = \'open\' AND `assignedto` != \'waiting\' AND `isDisputed` = \'no\''.mswSQLDepartmentFilter($ticketFilterAccess).' '.$filterBy),$msg_open); ?></p>
  </div>
...
<div class="contentWrapper"> 
  ...
  <div id="filterWrapper">
    ...
    <div class="filterRight">
      <p>
        <select onchange="location=this.options[this.selectedIndex].value">
        <option value="?p=open<?php echo (isset($_GET['dept']) ? '&amp;dept='.$_GET['dept'] : ''); ?>"><?php echo $msg_open3 ; ?></option>
        <?php
        foreach ($ticketLevelSel AS $k => $v) {
        ?>
        <option value="?p=open&amp;priority=<?php echo $k.(isset($_GET['dept']) ? '&amp;dept='.$_GET['dept'] : ''); ?>"<?php echo (isset($_GET['priority']) && $_GET['priority']==$k ? ' selected="selected"' : ''); ?>><?php echo $v; ?></option>
        <?php
        }
        ?>
        <option value="?p=open&amp;status=visitor<?php echo (isset($_GET['dept']) ? '&amp;dept='.$_GET['dept'] : ''); ?>"<?php echo (isset($_GET['status']) && $_GET['status']=='visitor' ? ' selected="selected"' : ''); ?>><?php echo $msg_open12; ?></option>
        <option value="?p=open&amp;status=admin<?php echo (isset($_GET['dept']) ? '&amp;dept='.$_GET['dept'] : ''); ?>"<?php echo (isset($_GET['status']) && $_GET['status']=='admin' ? ' selected="selected"' : ''); ?>><?php echo $msg_open11; ?></option>
        <option value="?p=open&amp;status=start<?php echo (isset($_GET['dept']) ? '&amp;dept='.$_GET['dept'] : ''); ?>"<?php echo (isset($_GET['status']) && $_GET['status']=='start' ? ' selected="selected"' : ''); ?>><?php echo $msg_open27; ?></option>
        </select>
      </p>
    </div>
...
<div id="ticketsWrapper">
  ...
  <div class="ticketWrapper<?php echo ($TICKETS->priority=='high' ? '_high' : ''); ?>">
    ...
    <div class="view">
      <p><a href="?p=view-ticket&amp;id=<?php echo $TICKETS->id; ?>"><img src="templates/images/view-ticket.png" alt="<?php echo $msg_open7.' #'.mswTicketNumber($TICKETS->id); ?>" title="<?php echo mswSpecialChars($msg_open7).' #'.mswTicketNumber($TICKETS->id); ?>" /></a></p>
    </div>
    
    <div class="info">
      <p>
      <?php echo str_replace(array('{ticket}','{date}','{priority}','{dept}','{status}','{replies}'),
                             array(mswTicketNumber($TICKETS->id),mswDateDisplay($TICKETS->ts,$SETTINGS->dateformat),mswGetPriorityLevel($TICKETS->priority),
                                   ...
                                   number_format(mswRowCount('disputes WHERE `ticketID` = \''.$TICKETS->id.'\''))
                             ),
                             $msg_open8); 
      ?>
      </p>
