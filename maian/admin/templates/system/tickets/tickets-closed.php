...
$closedCount = mswRowCount('tickets WHERE `ticketStatus` != \'open\' AND `assignedto` != \'waiting\' AND `isDisputed` = \'no\' '.mswSQLDepartmentFilter($ticketFilterAccess).' '.$filterBy);
if (isset($OK)) {
  echo mswActionCompleted($msg_open21);
}

if (isset($OK2)) {
  echo mswActionCompleted($msg_open20);
}
?>

<form method="post" id="form" action="?p=close" onsubmit="return confirmMessage('<?php echo mswSpecialChars($msg_javascript); ?>')">
<div id="headWrapper">
  
  <div id="message">
   <p><?php echo str_replace('{count}',$closedCount,$msg_open17); ?></p>
  </div>
...
<div class="contentWrapper"> 
  ...
  <div id="filterWrapper">
    ...
    <div class="filterRight">
      <p>
        <select onchange="location=this.options[this.selectedIndex].value">
        <option value="?p=close<?php echo (isset($_GET['dept']) ? '&amp;dept='.$_GET['dept'] : ''); ?>"><?php echo $msg_open3 ; ?></option>
        <?php
        foreach ($ticketLevelSel AS $k => $v) {
        ?>
        <option value="?p=close&amp;priority=<?php echo $k.(isset($_GET['dept']) ? '&amp;dept='.$_GET['dept'] : ''); ?>"<?php echo (isset($_GET['priority']) && $_GET['priority']==$k ? ' selected="selected"' : ''); ?>><?php echo $v; ?></option>
        <?php
        }
        ?>
        </select>
...
<div id="ticketsWrapper">
  ...
  <div class="ticketWrapper<?php echo ($TICKETS->priority=='high' ? '_high' : ''); ?>">
    ...
    <div class="view">
      <p><a href="?p=view-ticket&amp;id=<?php echo $TICKETS->id; ?>"><img src="templates/images/view-ticket.png" alt="<?php echo mswSpecialChars($msg_open7).' #'.mswTicketNumber($TICKETS->id); ?>" title="<?php echo mswSpecialChars($msg_open7).' #'.mswTicketNumber($TICKETS->id); ?>" /></a></p>
    </div>
  ...
  <p class="buttonWrapperTickets">
    ...
    <input type="submit" class="delbutton" id="button" value="<?php echo mswSpecialChars($msg_open15); ?>" title="<?php echo mswSpecialChars($msg_open15); ?>" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
