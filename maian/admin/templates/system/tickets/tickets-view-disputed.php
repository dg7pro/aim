...
<div id="headWrapper">
  ...
  <div id="message">
   <p class="ticketLinkArea"><span class="float">
      ...
      ?>
      <a class="open" href="?p=view-dispute&amp;open=<?php echo $_GET['id']; ?>" title="<?php echo mswSpecialChars($msg_viewticket26); ?>"><?php echo $msg_viewticket26; ?></a> 
      <?php
      } else {
      ?>
      ...
      <a class="close" href="?p=view-dispute&amp;close=<?php echo $_GET['id']; ?>" title="<?php echo mswSpecialChars($msg_viewticket27); ?>"><?php echo $msg_viewticket27; ?></a>
      <a class="lock" href="?p=view-dispute&amp;lock=<?php echo $_GET['id']; ?>" title="<?php echo mswSpecialChars($msg_viewticket28); ?>"><?php echo $msg_viewticket28; ?></a>
      <a class="ticket" href="?p=view-dispute&amp;cdis=<?php echo $_GET['id']; ?>" onclick="return confirmMessage('<?php echo mswSpecialChars($msg_javascript); ?>')" title="<?php echo mswSpecialChars($msg_disputes4); ?>"><?php echo $msg_disputes4; ?></a>
      <?php
      }
      ?>
     </span>
     <?php echo str_replace('{ticket}',mswTicketNumber($_GET['id']),$msg_viewticket81); ?> (<span class="highlight"><?php echo mswGetTicketStatus($SUPTICK->ticketStatus,$SUPTICK->replyStatus); ?></span>)
   </p>
  </div>
  ...
  <div id="viewBoxes">
    ...
    <div id="viewRight">
     ...
     <div class="rightInner">
      ...
      <h2 class="add_dispute_users"><span class="float"><a href="?p=view-dispute&amp;disputeUsers=<?php echo $_GET['id']; ?>" title="<?php echo mswSpecialChars($msg_disputes8); ?>" onclick="$.GB_hide();$.GB_show(this.href, {height: <?php echo GREYBOX_HEIGHT; ?>,width: <?php echo GREYBOX_WIDTH; ?>,caption: this.title});return false;"><img src="templates/images/add.png" alt="<?php echo mswSpecialChars($msg_disputes8); ?>" title="<?php echo mswSpecialChars($msg_disputes8); ?>" /></a></span><?php echo $msg_disputes5; ?></h2>
     
      <div class="disputeArea">
       <p class="user_<?php echo $SUPTICK->disPostPriv; ?>" onclick="ms_enPostPriv('<?php echo $_GET['id']; ?>','0')" id="oru" title="<?php echo mswSpecialChars($msg_disputes14); ?>"><?php echo mswSpecialChars($SUPTICK->name); ?><span class="email"><?php echo $SUPTICK->email; ?></span></p>
       <p class="in"><?php echo $msg_disputes13; ?></p>
       <?php
       $q = mysql_query("SELECT * FROM ".DB_PREFIX."disputes WHERE `ticketID` = '{$_GET['id']}' ORDER BY `postPrivileges`,`userName`")
            or die((ENABLE_MYSQL_ERRORS ? mswMysqlErrMsg(mysql_errno(),mysql_error(),__LINE__,__FILE__) : MYSQL_DEFAULT_ERROR));
       if (mysql_num_rows($q)>0) {
       while ($D_USER = mysql_fetch_object($q)) {
       ?>
        <p class="user_<?php echo $D_USER->postPrivileges; ?>" id="ou_<?php echo $D_USER->id; ?>" title="<?php echo mswSpecialChars($msg_disputes14); ?>" onclick="ms_enPostPriv('<?php echo $_GET['id']; ?>','<?php echo $D_USER->id; ?>')"><?php echo mswSpecialChars($D_USER->userName); ?><span class="email"><?php echo $D_USER->userEmail; ?></span></p>
       <?php
       }
       ?>
       <?php
       ...
       ?>
       <p class="none"><?php echo str_replace('{name}',mswSpecialChars($SUPTICK->name),$msg_disputes7); ?></p>
       <?php
       }
       ?>
      </div>
     ...
     <div class="rightInner" style="margin-top:5px">
      ...
      <h2><span class="float"><a href="?p=edit-ticket&amp;id=<?php echo $_GET['id']; ?>" title="<?php echo mswSpecialChars($msg_script9); ?>" onclick="$.GB_hide();$.GB_show(this.href, {height: <?php echo GREYBOX_HEIGHT; ?>,width: <?php echo GREYBOX_WIDTH; ?>,caption: this.title});return false;"><img src="templates/images/edit.png" alt="<?php echo mswSpecialChars($msg_script9); ?>" title="<?php echo mswSpecialChars($msg_script9); ?>" /></a></span><?php echo $msg_disputes6; ?></h2>
...
<div id="ticketReplies">
  ...
  <div class="reply_<?php echo $REPLIES->replyType; ?>">
    ...
    <div class="info">
     <span class="edit"><?php echo ($arCount>0 ? '<a class="attachlink" href="?p=view-dispute&amp;t='.$REPLIES->ticketID.'&amp;attach='.$REPLIES->id.'" title="'.mswSpecialChars($msg_viewticket40).'" onclick="$.GB_hide();$.GB_show(this.href, {height: '.GREYBOX_HEIGHT_ATTACHMENTS.',width: '.GREYBOX_WIDTH_ATTACHMENTS.',caption: this.title});return false;">'.str_replace('{count}',$arCount,$msg_viewticket41).'</a>' : ''); ?><a href="?p=view-dispute&amp;id=<?php echo $_GET['id']; ?>&amp;edit=<?php echo $REPLIES->id; ?>" onclick="$.GB_hide();$.GB_show(this.href, {height: <?php echo GREYBOX_HEIGHT_REPLY; ?>,width: <?php echo GREYBOX_WIDTH_REPLY; ?>,caption: this.title});return false;" title="<?php echo mswSpecialChars($msg_script9); ?>" ><?php echo $msg_script9; ?></a><?php echo (USER_DEL_PRIV=='yes' ? ' | <a onclick="return confirmMessage(\''.mswSpecialChars($msg_javascript).'\')" href="?p=view-dispute&amp;id='.$_GET['id'].'&amp;delete='.$REPLIES->id.'" title="'.mswSpecialChars($msg_script8).'">'.$msg_script8.'</a>' : ''); ?>
...
<div id="replyArea">
  ...
  <form method="post" action="?p=view-dispute&amp;id=<?php echo $_GET['id']; ?>" enctype="multipart/form-data" onsubmit="return addResponseBox('<?php echo mswSpecialChars($msg_javascript96); ?>','<?php echo mswSpecialChars($msg_javascript97); ?>','<?php echo mswSpecialChars($msg_javascript98); ?>','<?php echo mswRowCount('responses'); ?>','<?php echo RESTR_RESPONSES; ?>','<?php echo LICENCE_VER; ?>')">
  <div class="replyInner">
    ...
    <div class="replyBoxArea" id="replyBoxArea">
      ...
      ?>
      <textarea name="comments" rows="15" cols="40" id="comments" tabindex="<?php echo (++$tabIndex); ?>"></textarea></p>
      ...
      <div class="replyFooter">
        ...
        <div class="attachments">
          ...
          ?>
          <span class="bx"><input type="file" class="box" name="attachment[]" tabindex="<?php echo (++$tabIndex); ?>" /></span>
          </p>
          <?php 
        ...
        <div class="options_dispute">
        
          <div class="status">
            <label><?php echo $msg_viewticket17; ?> <?php echo mswDisplayHelpTip($msg_javascript130); ?></label>
          ...
          <div class="response">
            <label><?php echo $msg_viewticket35; ?> <?php echo mswDisplayHelpTip($msg_javascript131); ?></label>
            <input type="radio" id="r1" name="response" value="yes" tabindex="<?php echo (++$tabIndex); ?>" /> <?php echo $msg_script4; ?> <input tabindex="<?php echo (++$tabIndex); ?>" id="r2" type="radio" name="response" value="no" checked="checked" /> <?php echo $msg_script5; ?>
          </div>
          
          <div class="mail">
            <label><?php echo $msg_viewticket18; ?> <?php echo mswDisplayHelpTip($msg_javascript132); ?></label>
            <input tabindex="<?php echo (++$tabIndex); ?>" type="radio" name="mail" value="yes" checked="checked" /> <?php echo $msg_script4; ?> <input tabindex="<?php echo (++$tabIndex); ?>" type="radio" name="mail" value="no" /> <?php echo $msg_script5; ?>
          </div>
          ...
          ?>
          <div class="merge">
            <label><?php echo $msg_viewticket84; ?> <?php echo mswDisplayHelpTip($msg_javascript133,'LEFT'); ?></label>
            <input tabindex="<?php echo (++$tabIndex); ?>" type="text" class="box" title="<?php echo mswSpecialChars($msg_javascript44); ?>" name="mergeid" id="mergebox" onclick="$.GB_hide();$.GB_show('?p=merge-ticket&amp;id=<?php echo $_GET['id']; ?>', {height: <?php echo GREYBOX_HEIGHT; ?>,width: <?php echo GREYBOX_WIDTH; ?>,caption: this.title});return false;" />
          </div>
          <?php
          */
          ?>
        ...
        <p class="buttonWrapper"> 
         ...
         <input type="hidden" name="isDisputed" value="yes" /><br />
         <input tabindex="<?php echo (++$tabIndex); ?>" class="button" type="submit" value="<?php echo mswSpecialChars($msg_viewticket13); ?>" title="<?php echo mswSpecialChars($msg_viewticket13); ?>" />
         <input tabindex="<?php echo (++$tabIndex); ?>" onclick="ms_ticketPreview('view-ticket','comments')" class="button" type="button" value="<?php echo mswSpecialChars($msg_viewticket55); ?>" title="<?php echo mswSpecialChars($msg_viewticket55); ?>" style="margin-left:30px" />
        </p>
