...
function dbBackup($filepath,$compress=false) {
...
function mswGetTableData($table,$hexValue = true) {
	if (!dbBackup::setDatabase($this->database)) {
		return false;
  }
...
function doDump() {
  ...
  $header .= '# Maian Support Version: v'.SCRIPT_VERSION.mswDefineNewline();
  $header .= '# Date Created: '.mswDateDisplay(0,$SETTINGS->dateformat).' @ '.mswTimeDisplay(0,$SETTINGS->timeformat);
